...
function Navbar() {
  ...
  return (
    <nav className="glass-nav py-4 px-6 md:px-12 flex justify-between items-center transition-all duration-300 fixed top-0 w-full z-50">
      ...
      {/* Logo */}
      <Link href="/" className="font-serif text-2xl md:text-3xl font-medium tracking-tight text-text-main">
        GLOW & BEAUTY
      </Link>
      
      {/* Desktop Nav Links */}
      <div className="hidden md:flex items-center space-x-8">
        <Link href="/" className="text-sm font-medium tracking-wide hover:text-brand-gold transition-colors">HOME</Link>
        <Link href="/shop" className="text-sm font-medium tracking-wide hover:text-brand-gold transition-colors">SHOP ALL</Link>
      </div>
